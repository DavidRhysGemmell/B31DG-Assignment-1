...
void loop() {
 ...
 unsigned long currentMicros=millis(); //Starts a timer which starts at 0 when the ESP 32 is turned on. We compare other timings to this to detect when certain time has elapsed.
...
////Check Switch Settings at start of every cycle////
 if (currentMicros-PreviousMicrosWaveform==0){ //At the start of every cycle it will check what settings have been decided for next. I have seperated these so that the change occurs in the next waveformcycle and does not interupt the current cycle.
  NextCycle1=Switch1Setting;
...
////SIG A////
 if (NextCycle1==0){ //Switch1Setting was 0 at the start of this cycle. NO "else" nescessary, else would mean Switch1Setting=1 -> disables RedLED pulses, and RedLED is set to LOW at the beginning/end of every cycle.
   ...
   } else { //// NextCycle1=0, NextCycle2=1 -> Modified Waveform of SIG A. //// //Repeats until the WaveformCycle has completed//
    
     if (currentMicros-PreviousMicrosWaveform==50){ //Done 50uS signal for SIG B, now ready to start SIG A
        PreviousMicrosa=currentMicros; // Saves the time we start the SIG A so that we can compare the intervals so we know when to turn LED on.
        aPulseLength=Maxa; // This time we set aPulseLength as the maximum value, and will get smaller for each loop.
     }
     if (currentMicros-PreviousMicrosWaveform>=50){ //Done 50uS signal, now ready to start SIG A
       if (currentMicros-PreviousMicrosa<=aPulseLength){ // Checks with the timings to see if RedLED is should be on compared to the current pulse length
        
         if (currentMicros-PreviousMicrosWaveform<BlockLength) { //Checks to see if there whether the pulses have "intruded" into the d part of waveform
             digitalWrite(RedLED,HIGH); //Turns on LED for the pulse length
         } else {
             digitalWrite(RedLED,LOW); // for the values of intervals over the block length, RedLED is off. This is for d.
         }
       } else {
           digitalWrite(RedLED,LOW); // LED is off for the gap between pulses; b
       }  
       
       if (currentMicros-PreviousMicrosa>=(aPulseLength+b)){ // after the "a" pulse length plus "b" for LED being off, we reset
           PreviousMicrosa=currentMicros;
           aPulseLength=aPulseLength-50; //takes away 50 from each pulse length     
       }      
     }  
   }
